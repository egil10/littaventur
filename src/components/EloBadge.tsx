...
function Sparkline({ data }: { data: number[] }) {
  if (data.length < 2) {
    return (
      <div className="h-24 grid place-items-center text-sm text-ink-muted">
        Spill noen runder for å se utviklingen.
      </div>
    );
  }
  const w = 320;
    ...
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-24" preserveAspectRatio="none">
      <defs>
        <linearGradient id="elo-fill" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="rgba(10,108,255,0.22)" />
          <stop offset="100%" stopColor="rgba(10,108,255,0)" />
        </linearGradient>
      </defs>
      <path d={area} fill="url(#elo-fill)" />
      <path d={d} fill="none" stroke="var(--accent)" strokeWidth={2.25} strokeLinejoin="round" strokeLinecap="round" />
      <circle cx={pts[pts.length - 1][0]} cy={pts[pts.length - 1][1]} r={3.5} fill="var(--accent)" />
    </svg>
  );
}

// History panel for the per-device rating. Opened from the feedback card.
export default function EloHistoryModal({
  elo,
  onReset,
  onClose,
}: {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center p-4 frost-backdrop animate-fade-in" onClick={onClose}>
      <div className="frost liquid rounded-[28px] w-full max-w-md p-6 animate-pop" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <Activity size={18} />
            <h2 className="text-lg font-bold leading-tight">Din rating</h2>
          </div>
          <button onClick={onClose} className="pill-ghost focus-ring -mr-2 !px-2" aria-label="Lukk">
            <X size={18} />
          </button>
        </div>

        <div className="flex items-end gap-3 mb-4">
          <div className="text-5xl font-bold tabular-nums leading-none text-accent">{elo.rating}</div>
        ...
        <button onClick={onReset} className="pill-ghost focus-ring text-ink-muted">
          <RotateCcw size={15} />
          Nullstill rating
        </button>
      </div>
